...
# PC 1 and PC 2
if(has_colormap) {
  p <- ggplot(dat, aes(PC1, PC2, color=group)) +
...
if(has_colormap) {
  ...
  if("projected_samples" %in% colormap$group){
    temp_colormap <- tibble(
      group = c(colormap$group[colormap$group != "projected_samples"], "projected_samples"),
      color = c(rep("#666666",length(colormap$group) - 1), "#1f78b4"),
    )

    dat$order <- ifelse(dat$group == "projected_samples", '1', '0')
    dat <- arrange(dat, order) # arrange so that the projected samples are last
    ...
    p <- ggparcoord(dat,
                    ...
                    alphaLines = 0.5,
                    scale = "uniminmax") +
      scale_color_manual(values = temp_colormap$color, breaks = temp_colormap$group) +
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
      xlab("PC") +
      ylab("")
