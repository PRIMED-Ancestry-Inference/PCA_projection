...
if(npr %in% c(1,2,3,4,5)) {
  p <- ggpairs(dat, mapping = aes(color = .data[[group]], alpha = 0.5), columns = 1:npr)
  ggsave("out_file_pairs.png", plot = p, width=8, height=8)
  
  print(p)
} else if(npr > 5) {
  p1 <- ggpairs(dat, mapping = aes(color = .data[[group]], alpha = 0.5), columns = 1:5)
  p2 <- ggpairs(dat, mapping = aes(color = .data[[group]], alpha = 0.5), columns = 6:npr)
  ggsave("out_file_pairs.png", plot = p1, width=8, height=8)
